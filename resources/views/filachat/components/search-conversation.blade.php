...
<x-filament::modal width="xl" id="search-conversation">
    ...
    <x-filament::input.wrapper suffix-icon="heroicon-m-magnifying-glass">
        <x-filament::input type="search" placeholder="{{__('Search any messages...')}}"
            ...
            <ul class="absolute z-10 -mt-2 w-full bg-white border divide-y dark:divide-gray-800 border-gray-200 rounded-lg shadow dark:border-gray-800 dark:bg-gray-900 max-h-64 overflow-y-auto">
                @foreach($messages as $message)
                    <li wire:key="{{ $message->id }}">
                        <a wire:navigate
                           href="{{ FilaChat::getUrl(tenant: filament()->getTenant()) . '/' . $message->conversation->id }}">
                            <div class="m-1">
                                <div
                                    class="flex items-center rounded-lg gap-2 w-full p-3 hover:bg-gray-100 dark:hover:bg-white/5">
                                    <x-filament::avatar
                                        src="https://ui-avatars.com/api/?name={{ urlencode($message->other_person_name) }}"
                                        alt="Profile" size="lg" />
                                    <div class="flex flex-col w-0 flex-1">
                                        <div
                                            class="flex flex-col-reverse sm:flex-row items-start sm:items-center sm:justify-between">
    ...
    @elseif(!empty($search))
        <div class="relative">
            <div
                class="absolute z-10 w-full bg-white border dark:divide-gray-800 border-gray-200 rounded-lg shadow dark:border-gray-800 dark:bg-gray-900 max-h-64 overflow-y-auto">
